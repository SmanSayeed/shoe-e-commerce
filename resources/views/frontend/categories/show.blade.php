        ...
        <div class="mb-8">
            <div class="flex items-center space-x-4 overflow-x-auto pb-2">
                <a href="{{ route('categories.show', $category->slug) }}"
                   class="whitespace-nowrap px-4 py-2 rounded-lg border {{ !request('subcategory') ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }} transition">
                    All Products
                </a>
                @foreach($category->subcategories as $subcategory)
                <a href="{{ route('categories.show', $category->slug) }}?subcategory={{ $subcategory->slug }}"
                   class="whitespace-nowrap px-4 py-2 rounded-lg border {{ request('subcategory') === $subcategory->slug ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }} transition">
                    {{ $subcategory->name }}
                    <span class="text-xs opacity-75">({{ $subcategory->products()->count() }})</span>
                </a>
                @endforeach
            </div>
        </div>
