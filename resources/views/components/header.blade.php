        ...
        <div class="flex items-center gap-2 sm:gap-3 lg:gap-4">
            ...
            <div class="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                <button id="nav-toggle"
                    class="p-2 -ml-2 rounded-lg hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-offset-2 md:hidden transition-all duration-200"
                    style="--tw-ring-color: {{ $primaryColor }};" aria-label="Open Menu" onclick="triggerNavDrawer()">
                ...
                <a href="{{ route('home') }}" class="flex items-center group relative"
                    ...
                    @if ($logoUrl)
                        ...
                        <div
                            class="relative overflow-hidden rounded-lg transition-all duration-300 group-hover:scale-105 group-hover:shadow-md">
                        ...
                        <!-- Website name - visible on all screens, 2x bigger on desktop -->
                        <span
                            class="ml-2 text-xl sm:text-sm lg:text-xl font-semibold text-slate-900 group-hover:text-slate-700 transition-colors duration-200">
                            {{ $websiteName }}
                        </span>
                    @else
                        ...
                        <div class="flex items-center gap-2">
                            <!-- Logo Badge -->
                            <div class="relative">
                                <div class="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-sm"
                                    style="background: linear-gradient(135deg, {{ $primaryColor }}, {{ $accentColor }});">
                                </div>
                                <div class="relative flex items-center justify-center h-10 w-10 sm:h-12 sm:w-12 rounded-xl font-black text-white shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:shadow-xl group-hover:rotate-3"
                                    ...
                                    <span
                                        class="text-lg sm:text-xl">{{ strtoupper(substr($websiteName, 0, 1)) }}</span>
                                </div>
                            </div>
                            <!-- Website Name -->
                            <div class="flex flex-col">
                                <span
                                    class="text-xs sm:text-sm lg:text-xl font-bold text-slate-900 group-hover:text-slate-700 transition-colors duration-200 leading-tight">
                                    {{ $websiteName }}
                                </span>
                                @php
            ...
            <!-- Right-aligned search (expandable on mobile) -->
            <div class="hidden lg:flex w-[30%] mr-4" id="searchContainer">
                <form action="{{ route('products.search') }}" method="GET" class="relative w-full" data-search-form
                    data-search-placeholder-image="https://via.placeholder.com/96x96?text=No+Image">
                    <input type="search" name="q" value="{{ request('q') }}"
                        ...
                        style="--tw-ring-color: {{ $primaryColor }};" required autocomplete="off" data-search-input />
                    <button type="submit"
                        class="absolute right-2 top-1/2 -translate-y-1/2 text-white p-1.5 rounded-md transition-colors duration-200"
                        style="background-color: {{ $primaryColor }}; --tw-ring-color: {{ $primaryColor }};"
                        onmouseover="this.style.backgroundColor='{{ $accentColor }}'"
                    ...
                    <div class="absolute left-0 right-0 top-full mt-2 hidden rounded-2xl border border-slate-200 bg-white shadow-xl"
                        data-search-suggestions>
                        <div class="max-h-80 overflow-y-auto">
                            <ul class="divide-y divide-slate-100" data-search-suggestions-list></ul>
                        </div>
                        <div class="hidden px-4 py-3 text-sm text-slate-500" data-search-suggestions-empty>No results
                            found.</div>
                    </div>
                </form>
            </div>
            ...
            <button onclick="toggleSearch()"
                class="lg:hidden p-2 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-all duration-200">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="m21 21-4.35-4.35M17 10a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" />
                </svg>
            ...
            <div class="flex items-center gap-1 sm:gap-2 md:gap-4 flex-shrink-0">
                ...
                <a href="{{ route('cart.index') }}"
                    class="relative text-slate-700 hover:text-slate-900 p-1.5 sm:p-2 rounded-lg hover:bg-slate-100 transition-all duration-200 group">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                        viewBox="0 0 256 256" class="w-6 h-6">
                        <path
                    ...
                    <span
                        class="cart-count absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 text-white text-xs rounded-full h-4 w-4 sm:h-5 sm:w-5 flex items-center justify-center font-medium group-hover:scale-110 transition-transform duration-200"
                        style="background-color: {{ $accentColor }};" data-cart-count="0">0</span>
                </a>
            </div>
        </div>
    </div>
...
<script>
    ...
    function toggleSearch() {
        ...
        if (searchContainer) {
            ...
            if (searchContainer.classList.contains('hidden')) {
                ...
                searchContainer.classList.add('flex', 'fixed', 'inset-x-0', 'top-16', 'p-4', 'bg-white', 'shadow-md',
                    'z-50');
                // Focus the input
                ...
                searchContainer.classList.remove('flex', 'fixed', 'inset-x-0', 'top-16', 'p-4', 'bg-white', 'shadow-md',
                    'z-50');
            }
        }
    }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!isClickInside && searchContainer && !searchContainer.classList.contains('hidden') && window
            .innerWidth < 1024) {
            toggleSearch();
        }
    });
    ...
    function initSearchSuggestions() {
        ...
        forms.forEach((form) => {
            ...
            const placeholderImage = form.getAttribute('data-search-placeholder-image') ||
            ...
            const renderSuggestions = (items) => {
                ...
                items.forEach((item, index) => {
                    ...
                    const productUrl =
                        `${window.location.origin}/product/${encodeURIComponent(item.slug)}`;
                    ...
                    anchor.className =
                        'flex items-center gap-3 px-4 py-3 hover:bg-slate-50 focus:bg-slate-100 focus:outline-none';

                    const thumbnail = document.createElement('div');
                    thumbnail.className =
                        'h-12 w-12 rounded-lg bg-slate-100 overflow-hidden flex-shrink-0';
