        ...
        <div class="flex flex-col items-center justify-between gap-y-3 md:flex-row md:gap-y-0">
            ...
            <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
                ...
                <form method="GET" action="{{ route('admin.products.index') }}" class="flex items-center gap-2">
                    <div
                        class="group flex h-9 w-full items-center rounded-primary border border-transparent bg-white shadow-sm focus-within:border-primary-500 focus-within:ring-1 focus-within:ring-inset focus-within:ring-primary-500 dark:border-transparent dark:bg-slate-800 dark:focus-within:border-primary-500 md:w-64">
                        <div class="flex h-full items-center px-2">
                            <i class="h-3.5 text-slate-400 group-focus-within:text-primary-500"
                ...
                <form method="GET" action="{{ route('admin.products.index') }}" id="filterForm">
                    <div class="dropdown" data-placement="bottom-end">
                        <div class="dropdown-toggle">
                            <button type="button"
                                class="btn bg-white text-xs font-medium shadow-sm dark:bg-slate-800 h-9 px-3">
                                ...
                                <div>
                                    ...
                                    <select name="stock" class="input text-xs h-8">
                                        ...
                                        <option value="in_stock"
                                            {{ request('stock') == 'in_stock' ? 'selected' : '' }}>In Stock</option>
                                        <option value="out_of_stock"
                                            {{ request('stock') == 'out_of_stock' ? 'selected' : '' }}>Out of Stock
                                        </option>
                                        <option value="low_stock"
                                            {{ request('stock') == 'low_stock' ? 'selected' : '' }}>Low Stock</option>
                                    </select>
                                </div>
                                <div class="flex gap-2 pt-2">
                                    <button type="submit" class="btn btn-primary btn-sm text-xs flex-1">Apply</button>
                                    <a href="{{ route('admin.products.index') }}"
                                        class="btn btn-secondary btn-sm text-xs">Clear</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="dropdown-content w-56">
                        <ul class="dropdown-list p-2">
                            @php
                                ...
                                $createdDirection =
                                    $currentSort == 'created' && $currentDirection == 'desc' ? 'asc' : 'desc';
                                $priceDirection =
                                    request('sort') == 'price' && request('direction') == 'desc' ? 'asc' : 'desc';
                                $stockDirection =
                                    request('sort') == 'stock' && request('direction') == 'desc' ? 'asc' : 'desc';

                                // Build query arrays
                                $createdQuery = array_merge($queryParams, [
                                    'sort' => 'created',
                                    'direction' => $createdDirection,
                                ]);
                                $priceQuery = array_merge($queryParams, [
                                    'sort' => 'price',
                                    'direction' => $priceDirection,
                                ]);
                                $stockQuery = array_merge($queryParams, [
                                    'sort' => 'stock',
                                    'direction' => $stockDirection,
                                ]);
                            @endphp
                            <li class="dropdown-list-item">
                                <a href="{{ request()->fullUrlWithQuery($createdQuery) }}"
                                    class="dropdown-link text-xs">
                                    <span>Created Date</span>
                                    @if (request('sort') == 'created')
                                        <i class="w-3.5"
                                            data-feather="{{ request('direction') == 'asc' ? 'arrow-up' : 'arrow-down' }}"></i>
                                    @endif
                                </a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="{{ request()->fullUrlWithQuery($priceQuery) }}"
                                    class="dropdown-link text-xs">
                                    <span>Price</span>
                            ...
                            <li class="dropdown-list-item">
                                <a href="{{ request()->fullUrlWithQuery($stockQuery) }}"
                                    class="dropdown-link text-xs">
                                    <span>Stock</span>
            ...
            <div class="flex items-center gap-2">
                ...
                <a class="btn btn-primary text-xs h-9 px-3" href="{{ route('admin.products.create') }}"
                    role="button">
                ...
                <tbody>
                    @forelse($products as $product)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50"
                            data-order-count="{{ $product->orderItems->count() }}">
                            <td class="py-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="avatar avatar-circle w-8 h-8">
                                        @if ($product->primaryImage())
                                            <img class="avatar-img" src="{{ asset($product->primaryImage()) }}"
                                                alt="{{ $product->name }}" />
                                        @else
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-1.5">
                                            <h6
                                                class="whitespace-nowrap text-xs font-medium text-slate-700 dark:text-slate-100 truncate">
                                                {{ $product->name }}
                                            </h6>
                                        ...
                                        <div class="mt-0.5">
                                            @if ($product->is_active)
                                                <span
                                                    class="badge badge-soft-success text-xs py-0 px-1.5">Active</span>
                                            @else
                                                <span
                                                    class="badge badge-soft-danger text-xs py-0 px-1.5">Inactive</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="py-2">
                                @if ($product->sale_price)
                                    <div>
                                        <span
                                            class="text-xs font-semibold text-primary-500">${{ number_format($product->sale_price, 2) }}</span>
                                        <br>
                                        <span
                                            class="text-xs text-slate-400 line-through">${{ number_format($product->price, 2) }}</span>
                                    </div>
                                @else
                                    <span
                                        class="text-xs font-semibold">${{ number_format($product->price, 2) }}</span>
                                @endif
                            </td>
                            <td class="py-2">
                                <div class="flex flex-col">
                                    <div class="flex items-center gap-1">
                                        ...
                                        <span
                                            class="text-xs text-slate-500">({{ $product->variants->count() }})</span>
                                    </div>
                                    @php
                                        $inStockVariants = $product->variants->where('stock_quantity', '>', 0)->count();
                                        $lowStockVariants = $product->variants
                                            ->where('stock_quantity', '>', 0)
                                            ->where('stock_quantity', '<=', $product->min_stock_level ?? 5)
                                            ->count();
                            ...
                            <td class="py-2">
                                <span
                                    class="badge badge-soft-primary text-xs py-0.5 px-1.5">{{ $product->brand->name ?? 'N/A' }}</span>
                            </td>
                            <td class="py-2">
                                <span
                                    class="badge badge-soft-secondary text-xs py-0.5 px-1.5">{{ $product->category->name ?? 'N/A' }}</span>
                            </td>
                            <td class="py-2">
                                    ...
                                    <div class="dropdown" data-placement="bottom-start">
                                        ...
                                        <div class="dropdown-content">
                                            <ul class="dropdown-list">
                                                ...
                                                <li class="dropdown-list-item">
                                                    <button type="button"
                                                        class="dropdown-link text-xs delete-product {{ $product->orderItems->count() > 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                        data-id="{{ $product->id }}"
                                                        data-name="{{ $product->name }}"
                                                        {{ $product->orderItems->count() > 0 ? 'disabled' : '' }}>
                        ...
                        <tr>
                            <td colspan="9" class="text-center py-6">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h6 class="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">No products
                                        found</h6>
                                    <p class="text-xs text-slate-400 dark:text-slate-500 mb-3">Get started by creating
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    <label for="per-page-select"
                        class="text-xs font-medium text-slate-600 dark:text-slate-300">Show:</label>
                    <select id="per-page-select" class="input text-xs h-8 w-20" onchange="changePerPage(this.value)">
                ...
                <nav class="pagination">
                    <ul class="pagination-list">
                        ...
                        @foreach ($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                            ...
                            @else
                                <li class="pagination-item">
                                    <a class="pagination-link text-xs"
                                        href="{{ $url }}">{{ $page }}</a>
                                </li>
                            @endif
                        @endforeach
                ...
                if (bulkDeleteBtn) {
                    bulkDeleteBtn.addEventListener('click', function() {
                        ...
                        const validCheckboxes = Array.from(selectedCheckboxes).filter(checkbox => {
                            const orderCount = parseInt(checkbox.getAttribute('data-order-count') ||
                                '0');
                            return orderCount === 0;
                        });
                        ...
                        if (validCheckboxes.length === 0) {
                            Swal.fire({
                                title: 'Cannot Delete',
                                text: 'None of the selected products can be deleted because they have orders associated with them.',
                                icon: 'warning',
                                confirmButtonColor: '#3b82f6'
                            });
                            return;
                        ...
                        if (validCheckboxes.length !== selectedCheckboxes.length) {
                            confirmText =
                                `${selectedCheckboxes.length - validCheckboxes.length} product(s) cannot be deleted because they have orders. ${confirmText}`;
                        }
