  ...
  <div class="space-y-4">
    <div class="card">
      <div class="card-header">
        ...
        <p class="card-subtitle">Configure Pusher credentials for real-time notifications. Get your credentials from <a href="https://dashboard.pusher.com/" target="_blank" class="text-primary-500 hover:underline">Pusher Dashboard</a> (Free plan available).</p>
      </div>
      ...
      <div class="card-body">
        ...
        <form action="{{ route('admin.notification-settings.update') }}" method="POST" class="space-y-4" id="notificationSettingsForm">
          @csrf
          @method('PUT')
          ...
          <div class="form-group">
            ...
            >
              <option value="ap1" {{ old('pusher_cluster', $settings->pusher_cluster) === 'ap1' ? 'selected' : '' }}>Asia Pacific (Mumbai) - ap1</option>
              <option value="ap2" {{ old('pusher_cluster', $settings->pusher_cluster) === 'ap2' ? 'selected' : '' }}>Asia Pacific (Singapore) - ap2</option>
              <option value="ap3" {{ old('pusher_cluster', $settings->pusher_cluster) === 'ap3' ? 'selected' : '' }}>Asia Pacific (Tokyo) - ap3</option>
              <option value="ap4" {{ old('pusher_cluster', $settings->pusher_cluster) === 'ap4' ? 'selected' : '' }}>Asia Pacific (Sydney) - ap4</option>
              <option value="eu" {{ old('pusher_cluster', $settings->pusher_cluster) === 'eu' ? 'selected' : '' }}>Europe (Ireland) - eu</option>
              <option value="us2" {{ old('pusher_cluster', $settings->pusher_cluster) === 'us2' ? 'selected' : '' }}>United States (Ohio) - us2</option>
              <option value="us3" {{ old('pusher_cluster', $settings->pusher_cluster) === 'us3' ? 'selected' : '' }}>United States (Oregon) - us3</option>
            </select>
      ...
      document.addEventListener('DOMContentLoaded', function () {
        ...
        if (testBtn) {
          testBtn.addEventListener('click', function () {
            ...
            btn.disabled = true;
            btn.innerHTML = '<i class="w-4 h-4 mr-2 animate-spin" data-feather="loader"></i>Testing...';
