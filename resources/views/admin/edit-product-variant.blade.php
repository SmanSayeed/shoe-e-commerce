<x-admin-layout title="Edit Product Variant">
    ...
    <div class="mb-6 flex flex-col justify-between gap-y-1 sm:flex-row sm:gap-y-0">
        ...
        <ol class="breadcrumb">
            ...
            @if ($variant->product_id)
                <li class="breadcrumb-item">
                    <a href="{{ route('admin.product-variants.index', ['product_id' => $variant->product_id]) }}">Product
                        Variants</a>
                </li>
            @endif
            <li class="breadcrumb-item">
    ...
    <div class="space-y-6">
                ...
                <div class="card-body">
                    <div class="flex items-center gap-4">
                        @if ($variant->product->primaryImage())
                            <img src="{{ asset($variant->product->primaryImage()) }}"
                                alt="{{ $variant->product->name }}" class="w-16 h-16 object-cover rounded-lg" />
                        @else
        ...
        @else
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title text-danger">Product Not Found</h6>
                </div>
                <div class="card-body">
                    <p class="text-sm text-slate-600 dark:text-slate-400">The product associated with this variant could
                        not be found. Product ID: {{ $variant->product_id ?? 'N/A' }}</p>
                </div>
            </div>
        @endif
            ...
            <div class="card-body">
                ...
                <div id="variantErrors"
                    ...
                    <div class="flex">
                        ...
                        <button type="button"
                            onclick="document.getElementById('variantErrors').classList.add('hidden')" class="ml-auto">
                            <i data-feather="x" class="h-4 w-4"></i>
                        </button>
                    </div>
                </div>

                <form id="editVariantForm" action="{{ route('admin.product-variants.update', $variant) }}"
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        <div class="space-y-2">
                            ...
                            <input type="text" id="variant_name" name="name"
                                class="input @error('name') is-invalid @enderror" placeholder="e.g., Small, Red, etc."
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="variant_sku" name="sku"
                                ...
                                placeholder="Unique SKU for this variant" value="{{ old('sku', $variant->sku) }}"
                                required />
                            @error('sku')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="variant_size" name="size_id"
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="variant_color" name="color_id"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" id="variant_price" name="price"
                                class="input @error('price') is-invalid @enderror" placeholder="0.00" step="0.01"
                                min="0" value="{{ old('price', $variant->price) }}" />
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" id="variant_sale_price" name="sale_price"
                                class="input @error('sale_price') is-invalid @enderror" placeholder="0.00"
                                step="0.01" min="0"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" id="variant_stock" name="stock_quantity"
                                class="input @error('stock_quantity') is-invalid @enderror" placeholder="0"
                                min="0" value="{{ old('stock_quantity', $variant->stock_quantity) }}"
                                required />
                        ...
                        @if ($variant->image)
                            <div class="space-y-2">
                                <label class="text-sm font-medium text-slate-600 dark:text-slate-400">Current Variant
                                    Image</label>
                                <div class="flex items-center gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="file" id="variant_image" name="image"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" id="variant_weight" name="weight"
                                class="input @error('weight') is-invalid @enderror" placeholder="0.00" step="0.01"
                                min="0" value="{{ old('weight', $variant->weight) }}" />
                            @error('weight')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="flex justify-end mt-6 gap-3">
                        <a href="{{ route('admin.product-variants.index') }}{{ $variant->product_id ? '?product_id=' . $variant->product_id : '' }}"
                    ...
                    <div>
                        ...
                        <div class="space-y-1 text-sm text-slate-600 dark:text-slate-400">
                            @if ($variant->sale_price)
                                ...
                                <p><strong>Regular Price:</strong> <span
                                        class="line-through">${{ number_format($variant->price, 2) }}</span></p>
                            @else
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                editVariantForm.addEventListener('submit', function(e) {
                    ...
                    // Show loading state
                    editVariantBtn.innerHTML =
                        '<i data-feather="loader" class="w-4 h-4 animate-spin"></i><span>Updating...</span>';
                    ...
                    fetch(editVariantForm.action, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                // Redirect back to variants list page after a short delay
                                setTimeout(() => {
                                    @if ($variant->product_id)
                                        window.location.href =
                                            '{{ route('admin.product-variants.index', ['product_id' => $variant->product_id]) }}';
                                    @else
                                        window.location.href =
                        ...
                        .finally(() => {
                            // Reset button state
                            editVariantBtn.innerHTML =
                                '<i data-feather="save" class="w-4 h-4"></i><span>Update Variant</span>';
